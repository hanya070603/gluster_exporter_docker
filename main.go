...
func NewExporter(hostname, glusterExecPath, volumesString string, containerName string, profile bool, quota bool) (*Exporter, error) {
...
func main() {
	...
	var (
		...
		glusterVolumes = kingpin.Flag("gluster.volumes", fmt.Sprintf("Comma separated volume names: vol1,vol2,vol3. Default is '%v' to scrape all metrics", allVolumes)).Default(allVolumes).String()
                containerName = kingpin.Flag("gluster.containername", "gluster docker container name").Default("/usr/sbin/gluster").String()
	...
	exporter, err := NewExporter(hostname, *glusterPath, *glusterVolumes, *containerName, *profile, *quota)
	if err != nil {
