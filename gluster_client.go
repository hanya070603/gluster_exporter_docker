...
func ExecVolumeInfo() (structs.VolumeInfoXML, error) {
	//args := []string{"docker","exec","-it","glusterfs","/usr/sbin/gluster","volume", "info"}
        args := "docker exec " + containerName + " /usr/sbin/gluster volume info --xml"
...
func ExecVolumeList() (structs.VolList, error) {
        //args := []string{"docker","exec","-it","glusterfs","/usr/sbin/gluster","volume", "info"}
        args := "docker exec " + containerName + " /usr/sbin/gluster volume list --xml"
        bytesBuffer, cmdErr := execGlusterCommand(args)
	if cmdErr != nil {
...
func ExecPeerStatus() (structs.PeerStatus, error) {
        //args := []string{"docker","exec","-it","glusterfs","/usr/sbin/gluster","volume", "info"}
        args := "docker exec " + containerName + " /usr/sbin/gluster peer status --xml"
        bytesBuffer, cmdErr := execGlusterCommand(args)
	if cmdErr != nil {
...
func ExecVolumeProfileGvInfoCumulative(volumeName string) (structs.VolProfile, error) {
        ...
        args := "docker exec " + containerName + " /usr/sbin/gluster volume profile " + volumeName + " info cumulative --xml"
        bytesBuffer, cmdErr := execGlusterCommand(args)
	if cmdErr != nil {
...
func ExecVolumeStatusAllDetail() (structs.VolumeStatusXML, error) {
        //args := []string{"docker","exec","-it","glusterfs","/usr/sbin/gluster","volume", "info"}
        args := "docker exec " + containerName + " /usr/sbin/gluster volume status all detail --xml"
        bytesBuffer, cmdErr := execGlusterCommand(args)
	if cmdErr != nil {
...
func ExecVolumeHealInfo(volumeName string) (int, error) {
        ...
        args := "docker exec " + containerName + " /usr/sbin/gluster volume heal " + volumeName + " info --xml"
        bytesBuffer, cmdErr := execGlusterCommand(args)
	entriesOutOfSync := 0
	if cmdErr != nil {
		return -1, cmdErr
...
func ExecVolumeQuotaList(volumeName string) (structs.VolumeQuotaXML, error) {
        ...
        args := "docker exec " + containerName + " /usr/sbin/gluster volume quota " + volumeName + " list --xml"
        bytesBuffer, cmdErr := execGlusterCommand(args)
	if cmdErr != nil {
